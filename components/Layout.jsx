...
import { Menu, X, Linkedin, Facebook, Twitter, Instagram, Youtube, Mail, MapPin, ChevronDown, ChevronRight, MessageCircle, Check } from 'lucide-react';
...
const Navbar = () => {
  ...
  const navLinks = [
    ...
    { 
      ...
      path: '/international',
      children: [
        { name: 'International Director', path: '/team?tab=international_director' },
        { name: 'Chairman Int. Council', path: '/team?tab=international_chairman' }
      ]
    },
  ...
  const headerClass = `fixed w-full z-50 transition-all duration-300 ${
    isScrolled ? 'bg-white shadow-md py-2 text-navy-900' : 'bg-white shadow-sm py-4 text-navy-900'
  }`;
    ...
    <header className={headerClass}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          <Link to="/" className="flex items-center space-x-3 relative z-50 group">
          ...
          <nav className="hidden xl:flex space-x-6 items-center">
            {navLinks.map((link) => (
              <div key={link.name} className="relative group h-full py-2">
                ...
                >
                  {link.name}
                  {link.children && <ChevronDown size={14} className="ml-1 group-hover:rotate-180 transition-transform duration-300" />}
                </Link>
                  ...
                  <div className="absolute left-0 top-full pt-4 w-64 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform group-hover:translate-y-0 translate-y-2">
                    <div className="bg-white shadow-xl border-t-2 border-blue-700 rounded-b-sm py-1 flex flex-col">
                      {link.children.map((child) => (
                        ...
                        >
                          {child.name}
                          <ChevronRight size={12} className="opacity-0 group-hover/item:opacity-100 transition-opacity text-blue-600" />
                        </Link>
                      ))}
                    </div>
                  </div>
    ...
    <footer className="bg-white border-t border-gray-200 font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          <div className="space-y-6">
            <h3 className="text-xl font-bold text-blue-700 font-sans">About Us</h3>
              ...
              <div className="w-24 h-24 relative flex-shrink-0">
                  <div className="w-full h-full rounded-full border-4 border-blue-600 flex items-center justify-center relative bg-white">
                      <div className="absolute inset-1 border border-dashed border-red-500 rounded-full animate-spin-slow opacity-50"></div>
                       <span className="font-serif font-bold text-3xl text-red-600">FTII</span>
                  </div>
              </div>
          ...
          <div className="space-y-6">
            <h3 className="text-xl font-bold text-blue-700 font-sans">Useful Links</h3>
              ...
              {[
                ...
                <li key={i}>
                  <Link to={link.path} className="flex items-center group">
                    ...
                    <span className="text-slate-600 hover:text-blue-700 font-bold text-sm transition-colors">{link.name}</span>
                  </Link>
                </li>
              ))}
            </ul>
           ...
           <div className="space-y-6">
            <h3 className="text-xl font-bold text-blue-700 font-sans">Useful Resources</h3>
            <ul className="space-y-4">
              {[
          ...
          <div className="space-y-6">
            <h3 className="text-xl font-bold text-blue-700 font-sans">Connect with Us</h3>
            <ul className="space-y-6">
              <li className="flex items-center">
                <div className="mr-3 shrink-0 text-red-500">
                    <Mail size={22} />
                </div>
                <span className="text-slate-900 font-bold text-sm">ftiiassociation@gmail.com</span>
              </li>
              <li className="flex items-start">
                ...
                <span className="text-slate-900 font-bold text-sm leading-relaxed">
                    B- 1/67, Indira Chowk, near Nanital Bank, New Kondli, Delhi-110096.
                </span>
              </li>
            </ul>
          </div>
      ...
      <div className="bg-blue-900 py-4 border-t border-blue-800 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row justify-between items-center">
          <p className="text-xs opacity-80 font-medium">© 2023 FTII | All Rights are Reserved</p>
          <div className="flex space-x-4 mt-2 md:mt-0">
             <Facebook size={18} className="hover:text-gold-400 cursor-pointer"/>
             <Twitter size={18} className="hover:text-gold-400 cursor-pointer"/>
             <Instagram size={18} className="hover:text-gold-400 cursor-pointer"/>
             <Linkedin size={18} className="hover:text-gold-400 cursor-pointer"/>
